...
base_dir = "/home/dennis/Media_HDD/ETH/Robot_Learning/preprocessed_diffusedrive_dataset"
...
class CarlaDataset(datasets.GeneratorBasedBuilder):
    ...
    def _info(self):
        if self.config.name == "unconditioned":
            ...
            if self.config.using_cmd:
                ...
                features.update({"cmd": datasets.Sequence(datasets.Array2D(shape=(1, 1), dtype=int),
                                                          self.config.waypoint_buffer_size)})
        ...
        if self.config.name == "unconditioned":
            ...
            for folder_name_weather in os.listdir(base_dir):
                ...
                for folder_name_route in os.listdir(folder_path):
                    ...
                    folder_path_rgb_front = os.path.join(folder_path_current, "rgb_front")
                    folder_path_measurements = os.path.join(folder_path_current, "measurements")
                    folder_path_ego = os.path.join(folder_path_current, "ego_data")
                    ...
                    for file_name in measure_dir_list:
                        ...
                        if file_id < self.config.waypoint_buffer_size or file_id >= len(
                                measure_dir_list) - self.config.waypoint_prediction_size:
                            continue
                        ...
                        current_img = Image.open(file_path_rgb_front)
                        action_list, waypoint_flag = self.get_action_list(file_id, folder_path_ego)
                        ...
                        if using_cmd:
                            cmd_list = self.get_cmd_list(file_id, folder_path_ego)
            ...
            for folder_name_weather in os.listdir(base_dir):
                ...
                for folder_name_route in os.listdir(folder_path):
                    ...
                    for file_name in measure_dir_list:
                        ...
                        action_list, waypoint_flag = self.get_action_list(file_path_measurements, file_id,
                                                                          folder_path_measurements)
                        high_level_cmd_gps = self.get_high_level_cmd_gps(file_id, folder_path_measurements)
                        if using_cmd:
                            cmd_list = self.get_cmd_list(file_path_measurements, file_id, folder_path_measurements)
                ...
                for folder_name_route in os.listdir(folder_path):
                    ...
                    for file_name in measure_dir_list:
                        ...
                        if (current_json.get('ego_x') is None):
                            current_waypoint_gps = numpy.array(
                            ...
                            high_level_cmd_gps = self.get_high_level_cmd_gps(file_id, folder_path_measurements)
                        # TODO: change this high_level_cmd last dim is command not orientation
                        high_level_cmd_ego = self.convert_high_level_cmd_to_ego(high_level_cmd_gps,
                                                                                current_waypoint_gps)
    ...
    def get_action_list(self, file_id, folder_path_ego):
        # NOT in use, since at every waypoint the ego pose will change, so have to save the whole horizon for each data point.
        if file_id < self.config.waypoint_buffer_size or file_id > len(
                os.listdir(folder_path_ego)) - self.config.waypoint_prediction_size:
            return None, False

        current_waypoint_ego = numpy.array([0.0, 0.0, 0.0])
        past_waypoint_ego, past_waypoint_flag = self.get_past_waypoint_ego(file_id, folder_path_ego)
        future_waypoint_ego, future_waypoint_flag = self.get_future_waypoint_ego(file_id, folder_path_ego)

        if not past_waypoint_flag or not future_waypoint_flag:
            return None, False
    ...
    def get_past_waypoint_ego(self, file_id, folder_path):
        ego_json = json.load(open(os.path.join(folder_path, str(file_id).zfill(4) + ".json")))
        ego_x = ego_json["ego_past_waypoints_x"]
        ego_y = ego_json["ego_past_waypoints_y"]
        ego_theta = ego_json["ego_past_waypoints_theta"]
        return numpy.array([ego_x, ego_y, ego_theta]).transpose()[:self.config.waypoint_buffer_size], True
    ...
    def get_future_waypoint_ego(self, file_id, folder_path):
        ego_json = json.load(open(os.path.join(folder_path, str(file_id).zfill(4) + ".json")))
        ego_x = ego_json["ego_future_waypoints_x"]
        ego_y = ego_json["ego_future_waypoints_y"]
        ego_theta = ego_json["ego_future_waypoints_theta"]
        return numpy.array([ego_x, ego_y, ego_theta]).transpose()[:self.config.waypoint_prediction_size], True
    ...
    def get_cmd_list(self, file_id, folder_path):
        ego_json = json.load(open(os.path.join(folder_path, str(file_id).zfill(4) + ".json")))
        current_cmd = [ego_json["current_command"]]
        past_cmds = ego_json["past_commands"][:self.config.waypoint_buffer_size]
        # future_cmds = ego_json["future_commands"][:self.config.waypoint_prediction_size]

        cmd_list = past_cmds[::-1] + current_cmd
        return cmd_list

    def get_past_cmd(self, file_id, folder_path):
        ego_json = json.load(open(os.path.join(folder_path, str(file_id).zfill(4) + ".json")))
        past_cmds = ego_json["past_commands"]
        return past_cmds[:self.config.waypoint_prediction_size]

    def get_future_cmd(self, file_id, folder_path):
        ego_json = json.load(open(os.path.join(folder_path, str(file_id).zfill(4) + ".json")))
        future_cmds = ego_json["future_commands"]
        return future_cmds[:self.config.waypoint_buffer_size]
...
if __name__ == '__main__':
    data_set = load_dataset("/home/dennis/Media_HDD/ETH/Robot_Learning/DiffuseDrive/diffuser/datasets/carla_dataset",
                            "decdiff", streaming=True, split="train")
    ...
    now = time.time()
    for i, n in enumerate(data_set.iter(1)):
        if (i > 10):
            break
    print(time.time() - now)